...
def findLinesWith(contents, term, endTerm = None):
    ''' Return all lines containing a certain sequence, and the line numbers where they were found.
        contents: array of lines of a file (e.g. tixi.h)
...
def createParams(params, outparams, outarray):
    ''' Helper to create the strings that are used in the def: line of the python wrapper and that are passed to the TIXI call.
...
def createMethodStubs(defs):
    ...
    for d in defs:
        ...
        methodName = d.method_name[4].lower() + d.method_name[5:] # remove "tixi" prefix and make first character lower case
...
if __name__ == '__main__':
    ''' The main entry point, need to be called in the folder where tixi.h resides '''
    ...
    if upload:
        ...
        path = os.path.join(".", "dist/tixiwrapper-2011-01.win32.exe")
        googlecode_upload.upload_find_auth(path, "tixi", "TIXI Python Wrapper Win32 Installer", user_name = username, password = password)
        path = os.path.join(".", "dist/tixiwrapper-2011-01.zip")
        googlecode_upload.upload_find_auth(path, "tixi", "TIXI Python Wrapper Source", user_name = username, password = password)
