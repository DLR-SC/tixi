...
* Changed: $Id: tixi77.c 4392 2011-02-03 08:52:55Z litz_ma $ 
*
...
void tixiCreateMatrix_f(const TixiDocumentHandle *handle,
                        ...
                        int *error,
                        int parentPathLength,
                        int matrixNameLength, int rowNameLength, int columnNameLength)
{

  char *cParentPath = NULL;
  char *cMatrixName = NULL;
  char *cRowName = NULL;
  char *cColumnName = NULL;

  cParentPath = makeCString(parentPath, parentPathLength);
  cMatrixName = makeCString(matrixName, matrixNameLength);
  cRowName = makeCString(rowName, rowNameLength);
  cColumnName = makeCString(columnName, columnNameLength);

  *error =
    tixiCreateMatrix(*handle, cParentPath, cMatrixName, cRowName, cColumnName, *nRows, *nColumns);
...
void tixiAddFloatMatrix_f(const TixiDocumentHandle *handle,
                          ...
                          int rowNameLength, int columnNameLength, int formatLength)
{
  ...
  *error = tixiAddFloatMatrix(*handle, cParentPath,
                              cMatrixName, cRowName, cColumnName,
                              *nRows, *nColumns, (StorageMode) *mode, array, cFormat);
  free(cParentPath);
  free(cMatrixName);
  free(cRowName);
  free(cColumnName);
...
void tixiGetFloatMatrix_f(const TixiDocumentHandle *handle,
                          ...
                          int *mode,
                          double *array,
                          int *error, int matrixPathLength, int rowNameLength, int columnNameLength)
{
  char *cMatrixPath = NULL;
  char *cRowName = NULL;
  char *cColumnName = NULL;

  cMatrixPath = makeCString(matrixPath, matrixPathLength);
  cRowName = makeCString(rowName, rowNameLength);
  cColumnName = makeCString(columnName, columnNameLength);

  *error = tixiGetFloatMatrix(*handle, cMatrixPath,
                              cRowName, cColumnName, *nRows, *nColumns, (StorageMode) *mode, array);
                         ...
                         int *error, int matrixPathLength, int rowNameLength, int columnNameLength)
{
  ...
  *error = tixiGetMatrixSize(*handle, cMatrixPath, cRowName, cColumnName, nRows, nColumns);
