...
int preOtherFunctions( void ) {
  ...
  return tixiOpenDocument  ( xmlInputFilename,   &inDocumentHandle)  != SUCCESS
      || tixiCreateDocument( outRootElementName, &outDocumentHandle) != SUCCESS;
}

int postOtherFunctions( void ) {

  return ( tixiCloseDocument( inDocumentHandle ) != SUCCESS )
      || ( tixiSaveDocument( outDocumentHandle, xmlOutputFilename) != SUCCESS )
      || ( tixiCloseDocument( outDocumentHandle  ) != SUCCESS );
}
